...
allowed-tools: Read, Write, Edit, MultiEdit, "Bash(mkdir *)", "Bash(cd * && gh issue list *)", Glob, Grep
---
...
プロジェクトの実態を確認する。プロジェクトの構成に応じて適切なコマンドで主要ディレクトリやパッケージ構成を確認する。

### 2. GitHub イシュー確認

GitHub イシューを確認し、未対応の課題や進行中のタスクを把握:

- `cd {PROJECT_DIR} && gh issue list --state open --json number,title,labels,url` でオープンイシューを取得
...
更新したドキュメントの一覧をユーザーに報告する。

> **注意**: `.ai-agent/` は WORKSPACE_DIR にあり PROJECT_DIR の git 管理外のため、コミットは行わない。
