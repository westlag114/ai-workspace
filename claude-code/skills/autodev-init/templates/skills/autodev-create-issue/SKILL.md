...
allowed-tools: Read, "Bash(cd * && gh issue list *)", mcp__github__issue_write, AskUserQuestion
---
...
2. **重複確認**:
   ...
   - `cd {PROJECT_DIR} && gh issue list --search "<キーワード>" --json number,title,url` で既存 Issue との重複がないか確認
