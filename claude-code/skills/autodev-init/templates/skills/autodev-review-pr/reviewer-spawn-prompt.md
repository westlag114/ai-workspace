...
- `cd {PROJECT_DIR} && gh pr view {PR_NUMBER} --json files` で変更ファイル一覧を取得
- `cd {PROJECT_DIR} && gh pr diff {PR_NUMBER}` で差分を取得
...
- ローカルにチェックアウトされていないファイルは `cd {PROJECT_DIR} && gh pr checkout` でチェックアウトするか、Read ツールで読み取る
