...
allowed-tools: Read, "Bash(git -C * branch --show-current)", "Bash(cd * && gh pr list *)", Glob, Grep, AskUserQuestion, TeamCreate, TeamDelete, Task, TaskCreate, TaskUpdate, TaskList, TaskGet, SendMessage, WebSearch
---
...
- `$ARGUMENTS` が空の場合:
  ...
  2. `cd {PROJECT_DIR} && gh pr list --head <branch-name> --json number,url --limit 1` で該当ブランチの PR を検索
