---
name: autodev-switch-to-default
...
allowed-tools: "Bash(git -C * checkout *)", "Bash(git -C * pull *)", "Bash(git -C * pull)", "Bash(git -C * branch *)", "Bash(git -C * rev-parse *)", "Bash(git -C * remote *)", "Bash(git -C * symbolic-ref *)", "Bash(git -C * status *)", "Bash(cd * && gh pr list *)", mcp__github__merge_pull_request
---
...
- `git -C {PROJECT_DIR} remote get-url origin` でリポジトリの owner/repo を取得する
- `git -C {PROJECT_DIR} branch --show-current` で現在のブランチ名を取得する
- `cd {PROJECT_DIR} && gh pr list --head <branch-name> --json number,state,url --limit 1` で現在のブランチの PR を検索する
...
```bash
git -C {PROJECT_DIR} checkout <デフォルトブランチ>
```
