---
name: autodev-import-review-suggestions-local
...
allowed-tools: Read, Write, Edit, MultiEdit, "Bash(git -C * branch --show-current)", "Bash(cd * && gh pr list *)", "Bash(cd * && gh pr view *)", "Bash(git -C * add *)", "Bash(git -C * commit *)", "Bash(git -C * push *)", "Bash(mkdir *)", "Bash(ls *)", Glob, mcp__github__pull_request_read, mcp__github__add_reply_to_pull_request_comment
---
...
最後に「まとめ: X 件修正、Y 件スキップでよいですか？」と確認する。
